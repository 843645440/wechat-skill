...
> **2026-07 起默认关闭。** 流水线主路径为「用户主题 + 思路」，见 [user-brief.md](user-brief.md) 与 `SKILL.md`。  
> 仅当账号档案 `topic_discovery.enabled=true` **且** 用户明确要求自动选题时，才允许按本文执行。  
> 日常 Agent **不得**因无主题而进入本文流程。

以下为历史自动热点逻辑，供例外开启时参考。

---

仅在（例外开启且）触发请求没有提供主题时执行。
