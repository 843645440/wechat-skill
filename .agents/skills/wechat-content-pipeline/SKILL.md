...
- 正文配图：`../baoyu-article-illustrator/SKILL.md`
- 封面：`../baoyu-cover-image/SKILL.md`
- 图片渲染：`../agnes-image-gen/SKILL.md`
...
按账号档案调用正文配图 Skill，明确“直接生成、不用确认”，输出到 `illustrations/` 并把相对引用插入 `article.md`。在 `cover/source.md` 保存最终标题和视觉摘要，用封面 Skill 快速生成 `cover/cover.png`。项目图片偏好固定为 `agnes-image-gen`；它从运行环境读取 `AGNES_API_KEY`，不得把 Key 写入提示词、命令参数、日志或产物。

正文配图后端不可用时将 `illustrate` 标为 `skipped` 后继续。封面不可用时也标为 `skipped`：账号配置了默认封面素材就记录 `default_thumb_media_id=true`，否则记录 `false`。**图片或封面不可用不得提前终止任务，必须继续完成随机主题、排版和 HTML 校验；只在第 8 步门禁阻止草稿上传。**
