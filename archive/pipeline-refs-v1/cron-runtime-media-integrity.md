...
提示词里的“避免重复”不是门禁。账号级持久历史 `work/<account>/topic-history.json`：

- 每次选题写入 `topic`、`event_focus`（一句话事件重点）、`selected_at`；历史文件与账号绑定，原子写入，并限制条目数量。
- Agent 选题前读取**近 7 天**的 `event_focus`，按语义判断是否同一核心事件；同一事件换标题、换说法、换角度仍拒绝。
- 无法确定是否重复时默认放行，避免误杀新选题。
- 不使用 30 天窗口、证据 URL 重合或字符 n-gram 相似度等机械阈值（已删除）。

回归测试至少包含：近 7 天历史正确提供给 Agent、新选题写入历史、重新初始化后历史仍保留。
