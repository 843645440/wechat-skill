...
- 流水线排障：先读 `work/<account>/current/job.json` 与 `pipeline-failure-triage.md`。字数 1500—4000；写前 rotation+shape；写后 humanize。**正文图：baoyu 分析 + 自有后端出图**；**封面：品牌名+品牌色+场景**（不默认画完整 Logo，不做视觉审图）。写作须有读者带走物（反白看一场），见 `wechat-tech-insight-writer`。
