...
**把热点发现、写作、原生 HTML 信息设计、排版和多账号草稿串成可复用工作流**
...
一个给 AI Agent（Claude Code / Codex / Cursor 等）用的公众号内容工具包。根 Skill 负责把已有 Markdown 排成**样式全内联、粘贴到公众号编辑器不掉格式**的 HTML；完整流水线使用固定渲染器一次完成正文和原生信息模块，另附科技深度写作、确定性封面和多账号草稿能力，可从选题一路生成到指定账号草稿箱。
...
- **原生信息设计**：提取 0—3 个观点、比较、流程或数据模块，由固定渲染器一次完成同主题正文排版；异常时降级纯正文，不重试、不生成正文图片。
...
完整流水线由 `pipeline_runtime.py begin/prepare/finish` 固定执行确定性阶段。Agent 不应为单篇文章创建临时渲染器、封面规格或视觉检查流程。

没有给选题时，它会先联网筛选最新可靠热点；给了选题则直接使用。每个账号只复用一个内部交接区 `work/<account>/current/`，不是文章档案库，新任务会覆盖旧临时产物。

1. **确定选题** — 使用外部主题，或由 Agent 联网比较热点并直接选出一个可核验的最佳角度，不等待人工确认。
2. **写作与核验** — 生成 `article.md`，将来源和日期内部记录到 `sources.md`。
3. **随机主题与一次排版** — 随机固定一套主题，提取 0—3 个观点、比较、流程或数据模块，由固定脚本一次生成正文 HTML；模块异常直接降级纯正文。
4. **确定性封面** — 自动生成标题分行和高亮规格，从两套 HTML/CSS 模板中选一套，用浏览器在 45 秒硬超时内生成唯一封面 PNG。
5. **机械质量检查** — 信息模块不做视觉检测，封面只检查规格、PNG 签名和尺寸。
6. **严格校验** — 清除 HTML 错误、警告和占位符。
7. **自动建草稿** — 校验通过后立即写入指定账号草稿箱，到此结束；人工只在草稿箱审核，流水线不公开发布。
...
1. **HTML 确定性封面（默认）**：项目使用 `wechat-html-cover`，提供 `editorial-ledger`（编辑报刊风）和 `kinetic-type`（动态字构风）两套模板。固定脚本从最终文章自动生成标题分行和重点词，再通过 Chrome/Chromium 生成 PNG。两套模板目前不绑定账号。它不调用图片模型，不需要图片 API Key，也不进行 AI 视觉检测。固定输出为：
