...
- **内容生产流水线**：`wechat-content-pipeline` 可联网发现热点（须带 hook/tension/reader_stakes），写作前 `history --rotation` + `shape` 做结构轮换；正文强情感主观并按需简介陌生主体；写后 humanize；**封面用生图 API**（不再 HTML/Chrome 封面）；随机主题后进草稿箱。正文字数硬门禁 1500—4000。
...
4. **确定性封面** — 自动生成两行或三行标题及安全字号，从三套独立 HTML/CSS 模板中选一套，用浏览器在 45 秒硬超时内生成唯一封面 PNG。
5. **机械质量检查** — 图片只做落盘/魔数等机械检查，**不做视觉/OCR 审图**；观感由作者在草稿箱核对。
